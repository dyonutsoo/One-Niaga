...
export function Automate({
  ...
  async function generate() {
    ...
    setError('');
    try {
      const res = await aiFetch('/api/automate', {
        method: 'POST',
      ...
      setError(`${t('automate.error')} (${e instanceof Error ? e.message === 'AI_LIMIT_REACHED' ? t('billing.exhausted') : e.message : 'unknown error'})`);
    } finally {
      setLoading(false);
    }
  }
    ...
    <div>
      ...
      <div className="px-4 sm:px-8 pb-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-xl p-5 space-y-3" style={{ background: 'white', border: `1px solid ${BORDER}` }}>
          ...
          <button onClick={generate} disabled={!canGenerate || loading || remaining === 0} className="mt-2 w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold text-white" style={{ background: canGenerate ? '#E8552F' : '#D8B5A6', opacity: loading ? 0.7 : 1 }}>
