...
import { callAgnes, parsePipedLines } from '@/lib/agnes';
...
export async function POST(req: NextRequest) {
  let body: { products?: Product[]; orders?: Order[]; lang?: UiLang; platforms?: string[] };
  try {
    body = await req.json();
  } catch {
  ...
  if (!connected.length) return NextResponse.json({ error: 'Connect a store before requesting recommendations.' }, { status: 400 });
  ...
  const priceIssues =
    products
      .filter((p) => priceSpread(Object.fromEntries(connected.map(pl => [pl.id, p.platformPrices[pl.id]]))) > 3)
      .map((p) => `- ${p.name}: price varies by RM ${priceSpread(Object.fromEntries(connected.map(pl => [pl.id, p.platformPrices[pl.id]]))).toFixed(2)} across platforms`)
      .join('\n') || 'None';
