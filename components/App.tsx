...
import { buildCompanionRecommendations } from '@/lib/companion';
import type {
  ChannelMapping,
...
function Workspace() {
  ...
  const { plan, activate, changePlan } = useBilling();
  ...
  const [completedCompanionIds, setCompletedCompanionIds] = useState<Set<string>>(() => new Set());
  ...
  function handleSignIn(email: string) {
    const normalizedEmail = email || 'demo@oneniaga.my';
    activate(normalizedEmail);
  ...
  const companionRecommendations = useMemo(
    () => buildCompanionRecommendations({ products, orders: linkedOrders, messages, linked, holds, now }),
    [products, linkedOrders, messages, linked, holds, now]
  );

  function openCompanionTarget(item: CompanionRecommendation) {
    if (item.productId && item.targetTab === 'automate') {
      setSelectedProductId(item.productId);
    }
    if (item.targetFocus) {
      goToWithFocus(item.targetTab, item.targetFocus);
      return;
    }
    handleSetTab(item.targetTab);
  }

  function approveCompanionAction(item: CompanionRecommendation) {
    if (completedCompanionIds.has(item.id)) return;

    if (item.actionType === 'draft-po') {
      handleDraftPO();
    } else if (item.actionType === 'reallocate-stock') {
      handleReallocate();
    } else if (item.actionType === 'liquidate-deadstock') {
      handleLiquidate();
    } else if (item.actionType === 'release-reservation') {
      handleForceRelease();
    } else if (item.actionType === 'fix-listing') {
      if (item.productId) setSelectedProductId(item.productId);
      setTab('automate');
      pushToast({
        tone: 'info',
        title: 'Listing fix prepared',
        description: 'Prototype action: OneNiaga opened the listing workspace so the seller can approve clearer copy before publishing.',
      });
    } else if (item.actionType === 'standardize-pricing') {
      if (item.productId) setSelectedProductId(item.productId);
      setTab('products');
      pushToast({
        tone: 'info',
        title: 'Pricing review queued',
        description: 'Prototype action: OneNiaga highlighted a price gap for seller-approved platform price cleanup.',
      });
    } else if (item.actionType === 'draft-reply') {
      setTab('messages');
      pushToast({
        tone: 'info',
        title: 'Reply queue opened',
        description: 'Prototype action: OneNiaga moved the oldest unreplied buyer message to the top of today\'s work.',
      });
    } else if (item.actionType === 'review-orders') {
      setTab('orders');
      pushToast({
        tone: 'info',
        title: 'Order review opened',
        description: 'Prototype action: OneNiaga surfaced awaiting orders before fulfilment delays affect ratings.',
      });
    }

    setCompletedCompanionIds((prev) => new Set(prev).add(item.id));
  }
        ...
        <div className="dashboard-frame min-h-screen w-full flex flex-col md:h-screen md:overflow-hidden md:flex-row">
          ...
          {tab === 'overview' && (
            <Overview
              ...
              messages={messages}
              setTab={handleSetTab}
              goToWithFocus={goToWithFocus}
              companionRecommendations={companionRecommendations}
              completedCompanionIds={completedCompanionIds}
              onApproveCompanion={approveCompanionAction}
              onOpenCompanion={openCompanionTarget}
            />
          )}
          ...
          {tab === 'products' && (
            <ProductsPage
              ...
              onEdit={goAutomateFor}
              onManageAi={() => setTab('subscription')}
              focus={productsFocus}
              setFocus={setProductsFocus}
              companionRecommendations={companionRecommendations}
              completedCompanionIds={completedCompanionIds}
              onApproveCompanion={approveCompanionAction}
              onOpenCompanion={openCompanionTarget}
          ...
          {tab === 'subscription' && <SubscriptionPage plan={plan} setPlan={changePlan} />}
