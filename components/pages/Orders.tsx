  ...
  return (
    <div className="relative">
      <button
        ...
        style={{ border: `1px solid ${allSelected ? BORDER : '#DDE6FA'}`, background: allSelected ? 'white' : '#EEF2FB', color: allSelected ? INK : '#506CC7' }}
      >
            ...
            <div className="space-y-1 max-h-56 overflow-y-auto">
              {options.map((opt) => (
                <label key={opt.id} className="flex items-center gap-2 text-[12.5px] py-1 cursor-pointer">
                  <input type="checkbox" checked={selected.has(opt.id)} onChange={() => toggle(opt.id)} style={{ accentColor: '#506CC7' }} />
  ...
  return (
    <div>
      ...
      {unlinkedPlatforms.length > 0 && (
        <div className="mx-4 sm:mx-8 mb-3 rounded-lg border px-4 py-2.5 text-[12.5px]" style={{ background: '#FFF4DE', borderColor: '#F3D48B', color: '#7A5613' }}>
