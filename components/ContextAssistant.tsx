...
export function ContextAssistant({ mode, products = [], orders = [], platforms, onNavigate }: { mode: 'sales' | 'priorities'; products?: Product[]; orders?: Order[]; platforms: PlatformId[]; onNavigate: (tab: TabId) => void }) {
  ...
  return <div className="ai-panel">
    ...
    {result?.priorities && <ol className="mt-4 space-y-3">{result.priorities.map((p, i) => <li key={i} className="rounded-lg bg-white/80 p-3 flex gap-3"><span className="text-sm font-semibold" style={{ color: '#506CC7' }}>{i + 1}</span><div><h4 className="text-sm font-semibold">{p.title}</h4><p className="text-xs text-slate-600 mt-1 leading-relaxed">{p.reason}</p></div></li>)}</ol>}
    {error && <p role="alert" className="text-xs text-rose-700 mt-3">{error}</p>}
    <div className="mt-4 flex flex-wrap items-center gap-3"><button onClick={generate} disabled={loading || platforms.length === 0} className="ai-button" style={{ background: remaining === 0 ? '#E8552F' : undefined }}>{loading ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} />}{loading ? t('assist.working') : remaining === 0 ? 'Upgrade to continue' : result ? t('assist.refresh') : t(`assist.${mode}Action`)}</button><span className="text-xs text-slate-500">{t('assist.cost')} · {remaining} {t('assist.left')}</span></div>
    {remaining === 0 && <button onClick={() => onNavigate('subscription')} className="text-xs text-slate-700 mt-3 underline">{t('billing.exhausted')}</button>}
